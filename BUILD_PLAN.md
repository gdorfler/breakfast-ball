...
- [x] 7.5 (e) Visual course list (want-to-play + played, browsable) — **interactive/visual layer**. Shared `CourseCard` component (photo-independent): a fixed image slot falls back to a stylized "pin" swatch echoing the map's own language — solid fairway circle = played (earned), hollow fairway-lite-outline circle = want-to-play (promise) — so a card is never empty/broken with zero course photography. `photoUrl` prop exists but is unused today; wiring real round photos later is a data change, not a redesign. Played cards show filled stars + date (most recent first, from the existing query order); want-to-play cards omit rating entirely (existing most-recently-added-first order). Whole card links to the course page except the trailing remove control on want-to-play. Verified via the dev-only `/dev/card` bench: no horizontal overflow, correct swatch colors for both states, long names truncate cleanly, missing city/state doesn't break row height
